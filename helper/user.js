...
export function validateSignupInfo(user) {
  ...
  if (!validateName(firstname)) {
    throw new Error('Firstname must contain only letters (A-Z, a-z) and spaces.');
  } else if (!validateName(lastname)) {
    throw new Error('Lastname must contain only letters (A-Z, a-z) and spaces.');
  } else if (!validateUsername(username)) {
    throw new Error('Username must contain only letters (A-Z, a-z) and numbers (0-9).');
